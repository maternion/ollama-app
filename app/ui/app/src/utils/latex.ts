/**
 * Text transforms to normalize math delimiters language models emit into the
 * `$...$` / `$$...$$` form that remark-math parses.
 *
 * Models routinely emit math in delimiters remark-math does not recognize
 * (LaTeX `\(...\)` / `\[...\]` brackets, `[/math]` / `[/inline]` tags), and
 * they write currency amounts (`$5`) that single-dollar math otherwise eats.
 * These helpers normalize that output to the `$...$` / `$$...$$` form
 * remark-math parses. Compose them: `normalizeMathDelimiters` first, then
 * `escapeCurrencyDollars`.
 *
 * Vendored from @assistant-ui/react-markdown
 * (https://github.com/assistant-ui/assistant-ui), MIT licensed:
 *
 *   MIT License
 *   Copyright (c) 2025 AgentbaseAI Inc.
 *   (see the project LICENSE for full text)
 *
 * Two modifications for this fork:
 *   - The file is self-contained (no package imports).
 *   - `rewriteOutsideCode` was kept as-is; it is the core code-aware walker.
 */
...
/**
 * Preprocess model output so remark-math with single-dollar math enabled renders
 * math correctly and leaves currency alone. Apply this to the raw content string
 * before it reaches the markdown parser. Composes {@link normalizeMathDelimiters}
 * (MathJax/custom-tag conversion, code-aware) with {@link escapeCurrencyDollars}
 * (pair-based currency disambiguation, code-aware).
 */
export function preprocessLaTeX(text: string): string {
  return escapeCurrencyDollars(normalizeMathDelimiters(text));
}